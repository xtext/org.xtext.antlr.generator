...
/**
 * Converts the Xtext grammar to an AntLR grammar runs the AntLR generator. Additionally generates some parser/lexer
 * related services
 * 
...
public class XtextAntlrGeneratorFragment extends AbstractAntlrGeneratorFragment {
	...
	public void generate(final Grammar grammar, XpandExecutionContext ctx) {
		...
		final String absoluteGrammarFileName = srcGenPath + "/" + getGrammarFileName(grammar, getNaming()).replace('.', '/')
				+ ".g";
...
//		}).start();
	...
	public String[] getExportedPackagesRt(Grammar grammar) {
		return new String[] { getNaming().basePackageRuntime(grammar) + ".parser.antlr",
		...
		BindFactory factory = new BindFactory()
				...
				.addTypeToProviderInstance(
						...
						"org.eclipse.xtext.parser.antlr.LexerProvider.create("
								+ getLexerClassName(grammar, getNaming()) + ".class)")
				.addConfiguredBinding(
						"RuntimeLexer",
						"binder.bind(" + Lexer.class.getName() + ".class)"
								+ ".annotatedWith(com.google.inject.name.Names.named("
								+ "org.eclipse.xtext.parser.antlr.LexerBindings.RUNTIME" + ")).to("
								+ getLexerClassName(grammar, getNaming()) + ".class)")
				.addTypeToType(ITokenDefProvider.class.getName(), AntlrTokenDefProvider.class.getName());
		if (containsUnorderedGroup(grammar))
			factory = factory
					.addTypeToType(IUnorderedGroupHelper.class.getName(), UnorderedGroupHelper.class.getName());
	...
	public Set<Binding> getGuiceBindingsUi(Grammar grammar) {
		return new BindFactory()
				.addTypeToType("org.eclipse.jface.text.rules.ITokenScanner",
						"org.eclipse.xtext.ui.editor.syntaxcoloring.antlr.AntlrTokenScanner")
				...
				.addTypeToType("org.eclipse.xtext.ui.editor.IDamagerRepairer",
						"org.eclipse.xtext.ui.editor.XtextDamagerRepairer")
				.addConfiguredBinding(
						"HighlightingLexer",
						"binder.bind(" + Lexer.class.getName() + ".class)"
								+ ".annotatedWith(com.google.inject.name.Names.named("
				...
				.addConfiguredBinding(
						...
						"binder.bind(" + ITokenDefProvider.class.getName() + ".class)"
								+ ".annotatedWith(com.google.inject.name.Names.named("
								+ "org.eclipse.xtext.ui.LexerUIBindings.HIGHLIGHTING" + ")).to("
								+ AntlrTokenDefProvider.class.getName() + ".class)").getBindings();
	}

	public static String getAntlrTokenFileProviderClassName(Grammar grammar, Naming naming) {
		return naming.basePackageRuntime(grammar) + ".parser.antlr." + GrammarUtil.getName(grammar)
